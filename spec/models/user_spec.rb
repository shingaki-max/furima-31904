...
RSpec.describe User, type: :model do
  ...
  it 'passwordは半角英数字を含めないと登録できない' do
    ...
    @user.valid?
    expect(@user.errors.full_messages).to include('Password must be both single-byte alphanumeric characters')
  end
  ...
  it 'ユーザー本名名字は、全角でないと登録できない' do
    ...
    @user.valid?
    expect(@user.errors.full_messages).to include('Name must be both single-byte alphanumeric characters')
  end
  ...
  it 'ユーザー本名名字のフリガナは、全角（カタカナ）でないと登録できない' do
    ...
    @user.valid?
    expect(@user.errors.full_messages).to include('Name must be full-width katakana')
  end
